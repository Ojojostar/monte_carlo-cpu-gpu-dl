...
def datesetgener(process):
    header = f"Process {process}: "
    ...
    for i in range(start,max_length+1):
        ...
        if(i%batch_size==0):
            if limiter:
                ...
                print(header,currnum, "percent of the way there! Time is now:", (e-s)/60/60, "hours")
                print(f"{header}current time is: {now}")
                print("{}now saving tsnowX_{}.pt\n".format(header,currnum) )
    ...
    cuda.synchronize()
    e = time.time()
    print(header,"done!!! Yuri!!! :yuristar:")
    print(header,'time', (e-s)/60/60, 'hours')
    return
...
if __name__ == "__main__": 
    ...
    print('All Processes are Done!!!!!!! :necoarc:', flush=True)
